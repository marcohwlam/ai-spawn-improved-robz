...
- NOTE: Tasks 1-2 are complete and the renorm approach (Tasks 3-4) was REJECTED. The
  authoritative work is the `# REWORK` section at the bottom (Tasks R1-R3). Where these
  Global Constraints conflict with REWORK, REWORK governs. In particular: `bot.lua` IS
  modified in Task R2 (LabelFlags sector now comes from the base tag, and the
  `SectorOwnMax`/`SectorEnemyMin` constants are removed).
